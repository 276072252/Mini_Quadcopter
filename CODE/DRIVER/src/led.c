...
void LED_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB,ENABLE);   //使能PB外设时钟 
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC,ENABLE);   //使能PC外设时钟
	...
	GPIO_ResetBits(GPIOC,GPIO_Pin_14); //USER_LED灯指示RGB初始化成功
	GPIO_SetBits(GPIOC,GPIO_Pin_13); 	 //BAT_LOW_LED灯 关闭
	GPIO_ResetBits(GPIOB,GPIO_Pin_9);  //RGB置零
}
